...
void imageInterpolate(float factor,int h, int w, unsigned char **imgCR, unsigned char **img2gR, unsigned char **imgCG, unsigned char **img2gG,unsigned char **imgCB, unsigned char **img2gB) {
    ...
    // Calcul pixeli rezulatati in functie de cei 4x4 pixeli inconjuratori
     for(int i=0; i<(int)(factor*h); i++)
        for(int j=0; j<(int)(factor*w); j++){
            for( int l = 0; l < 4; l++)
                for( int k = 0; k < 4; k++ )
                    arr[l][k]=(double)imgCR[(int)(i/factor)][(int) (j/factor)];
                
            img2gR[i][j] = (unsigned char)bicubicpol(min(((double)i - ((int)(i/factor)*factor))/factor,h),
    ...
    // Calcul pixeli rezulatati in functie de cei 4x4 pixeli inconjuratori
     for(int i=0; i<(int)(factor*h); i++)
        for(int j=0; j<(int)(factor*w); j++){
            for( int l = 0; l < 4; l++)
                for( int k = 0; k < 4; k++ )
                    arr[l][k]=(double)imgCG[(int)(i/factor)][(int) (j/factor)];
                
            img2gG[i][j] = (unsigned char)bicubicpol(min(((double)i - ((int)(i/factor)*factor))/factor,h),
                                                    min(((double)j - ((int)(j/factor)*factor))/factor,w),
                                                    arr);
        }
    ...
    // Calcul pixeli rezulatati in functie de cei 4x4 pixeli inconjuratori
     for(int i=0; i<(int)(factor*h); i++)
        for(int j=0; j<(int)(factor*w); j++){
            for( int l = 0; l < 4; l++)
                for( int k = 0; k < 4; k++ )
                    arr[l][k]=(double)imgCB[(int)(i/factor)][(int) (j/factor)];
                
            img2gB[i][j] = (unsigned char)bicubicpol(min(((double)i - ((int)(i/factor)*factor))/factor,h),
                                                    min(((double)j - ((int)(j/factor)*factor))/factor,w),
                                                    arr);
        }
}
...
int main(int argc, char *argv[]) {
    ...
    imageInterpolate(factor,h,w,imgR, img2gR, imgG, img2gG, imgB, img2gB);
    // imageInterpolate(factor,h,w,imgG, img2gG);
    // imageInterpolate(factor,h,w,imgB, img2gB);
    ...
    // Salvarea imaginii
    sprintf(outFile, "%s%.2fx%s","../../output/serial/",factor,inFile.data());
